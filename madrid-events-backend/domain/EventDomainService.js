...
class EventDomainService {
    static fromJSON(json) {
        return new Event({
            ...
            streetAddress: json.address?.area?.['street-address'] || '',
            latitude: json.location?.latitude || null,
            longitude: json.location?.longitude || null,
            organizationName: json.organization?.['organization-name'] || '',
            link: json.link || '',
            image: json.image,
            distrito: json.address?.district?.['@id']?.split('/').pop() || '',
            barrio: json.address?.area?.['@id']?.split('/').pop() || '',
