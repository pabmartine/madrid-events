...
class EventDomainService {
    ...
    static fromJSON(json) {
        return new Event({
            ...
            free: json.free === 1 || json.free === true,
            price: json.price || '',
            ...
            locality: json.address?.area?.locality || json.locality || '',
            postalCode: json.address?.area?.['postal-code'] || json['postal-code'] || '',
            streetAddress: json.address?.area?.['street-address'] || json['street-address'] || '',
            latitude: json.location?.latitude ?? json.latitude ?? null,
            longitude: json.location?.longitude ?? json.longitude ?? null,
            organizationName: json.organization?.['organization-name'] || json['organization-name'] || '',
            link: json.link || '',
            image: json.image,
            distrito: json.address?.district?.['@id']?.split('/').pop() || json.distrito || '',
            barrio: json.address?.area?.['@id']?.split('/').pop() || json.barrio || '',
